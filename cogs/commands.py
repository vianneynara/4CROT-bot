...
spam""",
    ...
    async def remember(self, ctx, sec, *, kata):
        ...
        if random.randrange(1, 5) == 2:
            await ctx.reply(
                f"Broo i forgor to say {kata} to {ctx.author.name}!",
    ...
    async def poll(self, ctx, *, votet):
        ...
        meseg = await ctx.send(
            f"VOTE!:white_check_mark::negative_squared_cross_mark:\n**{votet}**"
        )
...
his/her account age: {user.created_at.strftime("Day: %d | Month: %m | Year: %Y | Hour: %H | Minute: %M")}
    ...
    async def embedc(self, ctx, link):
        ...
        if (
            896617220673785887 or 883722818888536094 in ctx.author.roles
        ):  # untuk cek apakah builder/staff
    ...
    async def loli(self, ctx):
        await ctx.reply(
            "https://cdn.discordapp.com/attachments/799121511470202921/888036846909673512/caption.gif"
        )
    ...
    async def siapa(self, ctx, *, spa):
        ...
        await ctx.reply(
            f"si {randomMember.name}#{randomMember.discriminator} yang {spa}"
        )
    ...
    async def on_message(self, message):
        ...
        if "http" in message.content.lower():
            if (
                message.channel.guild.id == 880497534957658132
            ):  # jika message ada di server 4crot
                channel = self.bot.get_channel(
                    887908566382571580
                )  # log bot 4crot di 4crot
            elif (
                message.channel.guild.id == 520809362525257748
            ):  # jika message ada di server kub
                channel = self.bot.get_channel(
                    940812191198773288
                )  # log bot 4crot di kub
